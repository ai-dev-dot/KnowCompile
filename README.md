...
5. **写入 + 索引** — 按 `# 标题` 分割多页面输出，写入 `wiki/*.md`，upsert SQLite pages 表，分块重新嵌入存入 LanceDB（`type='page'`）
...
用户问题 → bge-m3 嵌入 → LanceDB 向量搜索 Top-30 块
        ...
        → LLM 基于上下文生成回答 → 返回 {answer, sources[]}
```
...
5. **LLM 生成** — 注入 system.md 规则，严格基于参考资料回答，引用来源
...
完全从规范数据（`wiki/` 和 `raw/` 目录下的文件）重建所有索引。确定性操作——可随时执行，不丢失知识。

### 四、浏览 / 搜索 / 图谱

- **Wiki 浏览** — 解析 `[[双向链接]]`，渲染反向链接面板，去中心化页面导航
- **全文搜索** — FlexSearch 中文分词索引，构建时扫描全部 wiki 页面
