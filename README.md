...
| 功能 | 状态 | 说明 |
|------|------|------|
| 资料摄入 | ✅ | 拖放文件 / 粘贴文本 / 网页链接，PDF 文本提取，Markdown 图片资产自动导入，文件校验，批量编译 |
| LLM 编译 | ✅ | 5 步增量编译管道 + 内容审查 + 审查模型可独立配置，实时进度反馈，Schema few-shot 示例 |
...
| v0.2.2 ✅ | 渲染层测试基础设施 + 45 个新测试 + IPC mock 工厂 |
| v0.2.3 ✅ | 资料摄入全面改造 — PDF 支持 + 文本粘贴 + URL 摄入 + 批量编译 + 文件校验 |
| v0.3.0 ✅ | 编译质量系统性提升 — few-shot 示例 + 验证器修复 + 图片资产 + 审查模型独立配置 |
| v0.4.0 | 定期全局重构、知识库健康检查 |
