...
| 参数 | 默认值 | 说明 | 影响 |
|------|--------|------|------|
| `chunk_size` | 500 | 文本分块大小（字） | 越小检索越精确但索引变大；越大语义越完整但可能漏掉细节。改后需重建索引 |
| `compile_similarity_threshold` | 0.75 | 候选页面最低相似度 | 越高越保守（只匹配高度相关页面，可能漏掉）；越低越激进（匹配更多页面，可能误判） |
| `compile_candidate_count` | 3 | 送入 LLM 的候选页面数 | 越多覆盖面越全但编译耗时和 Token 消耗增加；越少越快但可能漏掉该更新的页面 |
| `qa_similarity_threshold` | 0.65 | 问答检索最低相似度 | 越高回答更聚焦但可能缺少相关信息；越低覆盖面更广但可能引入噪音 |
| `qa_retrieval_count` | 30 | 初始检索块数 | 越多召回率越高但检索变慢；越少越快但可能遗漏相关内容 |
| `qa_final_context_count` | 8 | 送入 LLM 的最终块数 | 越多上下文越丰富但 Token 消耗增加；越少越省 Token 但回答可能不够全面 |
| `qa_context_max_tokens` | 3000 | 上下文窗口 Token 上限 | 受限于 LLM 模型的最大上下文；越大可利用更多信息，但需确保不超过模型限制 |

**设计原则：** 不改也能用，改了更好用。每个参数旁显示上述影响说明。
